...
export function addTdwrReflectivityLayer(map) {
    if (tdwrReflectivityLayer) {
        map.removeLayer(tdwrReflectivityLayer);
        tdwrReflectivityLayer = null;
    }
    tdwrReflectivityLayer = L.layerGroup();
    ...
    TDWR.forEach(station => {
        ...
        marker.on('click', function () {
            ...
            if (!wmsLayers[stationNameLower]) {
                ...
                const wmsLayer = L.tileLayer.wms(`https://opengeo.ncep.noaa.gov/geoserver/${stationNameLower}/ows`, {
                    layers: `${stationNameLower}_brefl`,
                    format: 'image/png',
                    transparent: true,
                    version: '1.3.0',
        ...
        tdwrReflectivityLayer.addLayer(marker);
        ...
        tdwrReflectivityLayer.addLayer(label);
    });

    tdwrReflectivityLayer.addTo(map);
}

export function removeTdwrReflectivityLayer(map) {
    if (tdwrReflectivityLayer) {
        map.removeLayer(tdwrReflectivityLayer);
        tdwrReflectivityLayer = null;
    }
