let currentSevereWarnLayer = null;
...
export async function addCurrentSevereWarnLayer(map) {

    // Use current UTC time in the URL
    const currentTime = getCurrentUtcIsoMinute();
    const url = `https://mesonet.agron.iastate.edu/cgi-bin/request/gis/watchwarn.py?accept=shapefile&at=${currentTime}Z&timeopt=2&limitps=yes&phenomena=SV&significance=W`;
    ...
    currentSevereWarnLayer = L.geoJSON(filteredGeojson, {
        style: function () {
            return {
        ...
        onEachFeature: function (feature, layer) {
            if (feature.properties) {
                const { WFO, ISSUED, EXPIRED, WINDTAG, HAILTAG } = feature.properties;
                const popupContent = `
                    <strong>WFO:</strong> ${WFO ?? ''}<br>
                    <strong>Issued:</strong> ${formatDateTime(ISSUED)}<br>
                    <strong>Expires:</strong> ${formatDateTime(EXPIRED)}<br>
                    <strong>Wind Tag:</strong> ${WINDTAG ?? ''}
                    <strong>Hail Tag:</strong> ${HAILTAG ?? ''}
                `;
...
export function removeCurrentSevereWarnLayer(map) {
    if (currentSevereWarnLayer) {
        map.removeLayer(currentSevereWarnLayer);
        currentSevereWarnLayer = null;
    }
}