let currentFlashFloodWarnLayer = null;
...
export async function addCurrentFlashFloodWarnLayer(map) {

    // Use current UTC time in the URL
    const currentTime = getCurrentUtcIsoMinute();
    const url = `https://mesonet.agron.iastate.edu/cgi-bin/request/gis/watchwarn.py?accept=shapefile&at=${currentTime}Z&timeopt=2&limitps=yes&phenomena=FF&significance=W`;
    ...
    currentFlashFloodWarnLayer = L.geoJSON(filteredGeojson, {
        style: function () {
            return {
        ...
        onEachFeature: function (feature, layer) {
            if (feature.properties) {
                const { WFO, ISSUED, EXPIRED } = feature.properties;
                const popupContent = `
...
export function removeCurrentFlashFloodWarnLayer(map) {
    if (currentFlashFloodWarnLayer) {
        map.removeLayer(currentFlashFloodWarnLayer);
        currentFlashFloodWarnLayer = null;
    }
}