...
let nexradReflectivityLayer = null;

export function addNexradReflectivityLayer(map) {
    if (nexradReflectivityLayer) {
        map.removeLayer(nexradReflectivityLayer);
        nexradReflectivityLayer = null;
    }
    nexradReflectivityLayer = L.layerGroup();
    ...
    NEXRAD.forEach(station => {
        ...
        marker.on('click', function () {
            ...
            if (!wmsLayers[stationNameLower]) {
                ...
                const wmsLayer = L.tileLayer.wms(`https://opengeo.ncep.noaa.gov/geoserver/${stationNameLower}/ows`, {
                    layers: `${stationNameLower}_sr_bref`,
                    format: 'image/png',
                    transparent: true,
                    version: '1.3.0',
        ...
        nexradReflectivityLayer.addLayer(marker);
        ...
        nexradReflectivityLayer.addLayer(label);
    });

    nexradReflectivityLayer.addTo(map);
}

export function removeNexradReflectivityLayer(map) {
    if (nexradReflectivityLayer) {
        map.removeLayer(nexradReflectivityLayer);
        nexradReflectivityLayer = null;
    }
